...
def list_attendance(db: Session = Depends(get_db)):
    ...
    records = db.query(AttendanceRecord).order_by(AttendanceRecord.date.desc(),AttendanceRecord.check_in_time.desc()).all()

    # fetch site timezones
    site_ids = set([r.check_in_site_id for r in records if r.check_in_site_id] +[r.check_out_site_id for r in records if r.check_out_site_id])
